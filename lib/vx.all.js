...
function vN(n){var r=arguments.callee,p=n.split('.');for(var i=0;i<p.length&&p[i];i++){if(r[p[i]]==undefined)r[p[i]]={};r=r[p[i]]}return r}
...
_('').A=function(h,p,s,e,r,x,f,i,b){return vAc(f,i,function(a){(a==1&&b)?b():0;h.style[p]=r+(s+(e-s)*a)+x})}
...
_('').S=function(j){if(!j)return j==undefined?'undefined':(j===null?'null':j);var f=arguments.callee,c=j.constructor.toString(),t=[];if(c.match(/array/i)){for(x in j)t.push(f(j[x]));return'['+t.join(',')+']'}if(c.match(/object/i)){for(x in j)t.push(x+':'+f(j[x]));return'{'+t.join(',')+'}'}if(c.match(/string/i))return"'"+j.replace(/\'/g,"\\'")+"'";return j}
